...
const PLUGIN_NAME = 'gulp-sentry-release'
...
const makeRequestParams = (apiOptions) => {
  ...
  return (path = '/', dataOptions = { body: {}, formData: {} }) => {
...
const validateOptions = options =>
  Object.keys(options)
    ...
    .forEach(key => validateRequired(options[key]))
...
const createHttpHandler = (done, successMsg, ignoreConflict) => (error, { statusCode }, body) => {
  if (error) {
